...
crypto::bytes sign(const crypto::bytes &message,
  ...
  unique_bn bn(BN_bin2bn(private_key.data(),
  ...
  if (EC_POINT_mul(group, pub_point.get(), bn.get(), nullptr, nullptr,
                   nullptr) != 1) {
...
bool verify(const crypto::bytes &message, const crypto::bytes &signature,
  ...
  if (EC_POINT_oct2point(group, pub_point.get(), public_key.data(),
