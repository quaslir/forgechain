...
crypto::bytes Block::serialize() const {
  ...
  out.insert(out.end(), reinterpret_cast<const uint8_t *>(&tx_count),
             reinterpret_cast<const uint8_t *>(&tx_count) + sizeof(tx_count));

  crypto::bytes tx_out;
  for (const auto &tx : transactions_) {
    bytes tx_serialized = tx.serialize();
    auto tx_len = static_cast<uint32_t>(tx_serialized.size());
    tx_out.insert(tx_out.end(), reinterpret_cast<const uint8_t *>(&tx_len),
                  reinterpret_cast<const uint8_t *>(&tx_len) + sizeof(tx_len));
    tx_out.insert(tx_out.end(), tx_serialized.begin(), tx_serialized.end());
  }
...
std::optional<Block> Block::deserialize(const crypto::bytes &payload) {
  ...
  size_t offset = 0;
  if (payload.size() < sizeof(uint32_t) + offset)
    return std::nullopt;

  uint32_t version = *reinterpret_cast<const uint32_t *>(payload.data());
  offset += sizeof(version);

  if (payload.size() < HASH_BYTES_SIZE + offset)
    return std::nullopt;
  crypto::HashBytes prev_hash;

  std::copy(payload.data() + offset, payload.data() + offset + HASH_BYTES_SIZE,
            prev_hash.data());
  ...
  if (payload.size() < sizeof(uint64_t) + offset)
    return std::nullopt;
  uint64_t timestamp =
  ...
  for (uint32_t i = 0; i < tx_count; i++) {
    ...
    if (payload.size() < offset + tx_len)
      return std::nullopt;
    crypto::bytes tx_payload(payload.data() + offset,
                             payload.data() + offset + tx_len);
