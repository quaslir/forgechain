...
namespace forgechain::core {
    Block::Block(uint32_t version, HashBytes prev_hash, uint64_t timestamp) : version_(version), prev_hash_(prev_hash), timestamp_(timestamp),
   ...
   crypto::bytes Block::serialize() const {
       crypto::bytes out;
        ...
        out.insert(out.end(), reinterpret_cast<const uint8_t *>(&timestamp_),
            reinterpret_cast<const uint8_t *>(&timestamp_) + sizeof(timestamp_));

        out.insert(out.end(), reinterpret_cast<const uint8_t *>(&difficulty_),
            reinterpret_cast<const uint8_t *>(&difficulty_) + sizeof(difficulty_));
