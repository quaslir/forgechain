...
namespace forgechain::core {
    ...
    std::optional<Block> OrphanPool::find_orphan(const crypto::HashBytes& hash) const {
        auto it = orphan_pool_.find(hash);
        if(it == orphan_pool_.end()) return std::nullopt;
        return it->second;
    }
     std::optional<Block> OrphanPool::find_orphan_by_prev_hash(const crypto::HashBytes& prev_hash) const {
    ...
    void OrphanPool::remove_orphan(const crypto::HashBytes& hash) {
     ...
     size_t OrphanPool::orphan_count() const {
         return orphan_pool_.size();
     }
}
