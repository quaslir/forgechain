...
namespace forgechain::app {
    RpcServer::RpcServer(network::Node& node, uint16_t port) : node_(node), port_(port) {}
    ...
    void RpcServer::handle_connection(network::TcpSocket socket) {
        ...
        while(network::read_exact(socket.fd(),&byte, 1)) {
            if(line.size() >= 4096) {
                ...
                network::send_exact(socket.fd(), reinterpret_cast<const uint8_t *>(error_response.data()), error_response.size());
                return;
            }
            if(byte == '\n') {
                break;
       ...
       network::send_exact(socket.fd(), reinterpret_cast<const uint8_t *>(result.data()), result.size());
    }
    ...
    crypto::str RpcServer::handle_command(const crypto::str &line) {
        ...
        if(command == "GETBALANCE") {
            crypto::str address{};
            iss >> address;
            if(address.empty()) return "ERROR_EMPTY_ADDRESS";
            ...
            crypto::str hex;
            iss >> hex;
            if(hex.empty()) return "ERROR_EMPTY_PAYLOAD";
