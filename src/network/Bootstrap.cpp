...
namespace forgechain::network {
    std::optional<std::vector<PeerAddress>> load_bootstrap_peers(const crypto::str& path) {
        ...
        while(file >> buffer) {
            ...
            if(delimeter_pos == crypto::str::npos) continue;
            crypto::str host = buffer.substr(0, delimeter_pos);
            crypto::str port_str = buffer.substr(delimeter_pos + 1);
            if(host.empty() || port_str.empty()) continue;
            bool all_digits = std::all_of(port_str.begin(), port_str.end(), [](unsigned char c) {
            ...
            try {
                ...
                if(port_num < 0 || port_num > 65535) continue;
                port = static_cast<uint16_t>(port_num);
                peer_addresses.push_back(PeerAddress{.host = std::move(host), .port = port});
            } catch(...) {
                continue;
            }
        }
