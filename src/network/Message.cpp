...
namespace forgechain::network {
    bool send_message(int fd, const Message& message) {
        ...
        header.insert(header.end(), length_bytes, length_bytes + sizeof(payload_length));
        if(!send_exact(fd, header.data(), HEADER_LENGTH)) return false;
    ...
    bool receive_message(int fd, Message& message) {
        ...
        if(!read_exact(fd, header_buffer.data(), HEADER_LENGTH)) return false;

        // check magic
        for(size_t i = 0; i < 4; i++) {
            if(header_buffer[i] != MAGIC_BYTES[i]) return false;
        }
        // fill type
       message.type = static_cast<MessageType>(header_buffer[4]);
        // fill size
        uint32_t length = *reinterpret_cast<const uint32_t*>(header_buffer.data() + 5);
