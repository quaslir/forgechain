...
void Storage::create_schema() {
  SqliteStatement blocks_statement{connection_,
  ...
  SqliteStatement balance_statement{connection_,
...
void Storage::save_block(const core::Block &block, size_t height) {
  SqliteStatement stmt(connection_, "INSERT OR REPLACE INTO blocks (height, "
                                    "hash, serialized) VALUES (?, ?, ?)");
  if (!stmt.is_valid()) {
...
std::optional<core::Block> Storage::load_block(size_t height) const {
  SqliteStatement stmt(connection_,
                       "SELECT serialized FROM blocks WHERE height = ?");
  if (!stmt.is_valid()) {
...
Storage::load_balance(const crypto::str &address) const {
  SqliteStatement stmt(connection_,
...
Storage::load_all_balances() const {
